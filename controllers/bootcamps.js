...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
  ...
  // Check for published bootcamp by that user
  const publishedBootcamp = await Bootcamp.findOne({ user: req.user.id });

  // If the user is not an admin, they(ie publisher) can only add one bootcamp
...
exports.updateBootcamp = asyncHandler(async (req, res, next) => {
  ...
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== "admin") {
    // id is stored as object in mongodb so we have to convert it to string first for comparision
    return next(
      new ErrorResponse(
  ...
  // only when all the validations are passed then update the bootcamp
