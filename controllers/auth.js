...
exports.updateDetails = asyncHandler(async (req, res, next) => {
  // if we use req.body as a filed to update then extra fields will also be updated so here we are extracting only name and email to update
...
exports.forgotPassword = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    // if email is not sent the reset these fileds to undefined
    user.resetPasswordToken = undefined;
    user.resetPasswordExpire = undefined;

    await user.save({ validateBeforeSave: false }); // doesn't run any validation like email and name
...
exports.resetPassword = asyncHandler(async (req, res, next) => {
  ...
  const user = await User.findOne({
    resetPasswordToken,
    resetPasswordExpire: { $gt: Date.now() }, // checking if the expiry date is greater than the present date
  });
